...
// platformSwitcherLnkPath returns the Desktop path for the "open this platform in SteamSwitch" shortcut.
func platformSwitcherLnkPath(platformKey string) (string, error) {
	...
	base := "SteamSwitch - " + sanitizeShortcutFileName(platformKey)
...
func CreatePlatformShortcut(platformKey string) (string, error) {
	...
	desc := fmt.Sprintf("SteamSwitch - %s", platformKey)
	argv := platformKey
