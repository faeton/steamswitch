...
// errUpdateChecksDisabled is returned when the fork has no release feed configured.
var errUpdateChecksDisabled = errors.New("update checks are disabled in this build")

const (
	failStateFile    = "SteamSwitch.lastUpdateCheckFail.json"
...
func FetchLatestVersion(ctx context.Context, client *http.Client, currentVersion string) (version string, message string, err error) {
	...
	// No release feed in this fork: updateAPIURL is empty, so never call out.
	if updateAPIURL(currentVersion) == "" {
		return "", "", errUpdateChecksDisabled
	}
...
// FetchLaunchAPICheck hits the legacy update API endpoint (disabled in this fork)
...
// RunLaunchAPICheck runs the legacy update API check (disabled in this fork).
