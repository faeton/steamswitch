...
type Parsed struct {
	...
	PassthroughLaunchArgs []string // forwarded to the target platform exe (not SteamSwitch flags)
...
// NormalizeProtocolArg strips steamswitch:// and returns the payload path (e.g. "s:765...").
func NormalizeProtocolArg(s string) string {
	s = strings.TrimSpace(s)
	low := strings.ToLower(s)
	if strings.HasPrefix(low, "steamswitch://") {
		s = s[len("steamswitch://"):]
	}
...
//
// `--page=<platform>` predates the redesign, when the account list lived behind a platform
// chooser. The account list is now the home route, so every platform name resolves there.
// The flag is still accepted so existing shortcuts and `steamswitch://` links keep working;
// emitting the old `{"page":"platform"}` would reach the frontend through `applyNavigateJSON`
// — which does not run the hash aliases — and land on a route with no page module.
