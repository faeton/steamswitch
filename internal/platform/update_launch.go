...
func emitAppUpdateAvailable(message string) {
	...
	const downloadURL = "https://github.com/faeton/steamswitch/releases/latest"
	_ = app.Event.Emit(AppUpdateAvailableEvent, UpdateAvailablePayload{
		Message:     message,
		DownloadURL: downloadURL,
	})
	NotifyNative("steamswitch-update-available", "SteamSwitch", message, map[string]interface{}{
...
func EnableAutoRestartAfterUpdate(app *application.App) {
	...
	app.Event.On(updater.EventUpdateAvailable, func(*application.CustomEvent) {
		NotifyNative("steamswitch-update-downloading", "SteamSwitch", "An update is available. Downloading it now.", map[string]interface{}{
			"type": "update-available",
		})
	})
	app.Event.On(updater.EventUpdateReady, func(*application.CustomEvent) {
		NotifyNative("steamswitch-update-ready", "SteamSwitch", "Update installed. Restarting SteamSwitch.", map[string]interface{}{
			"type": "update-ready",
		})
		go func() {
			defer crashlog.Capture()
...
// Uses the Wails signed GitHub updater first; falls back to the legacy update API (disabled in this fork).
