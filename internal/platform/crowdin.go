...
// crowdinAPIURL is empty in this fork: the translator credits list is served by
// tcno.co for the upstream Crowdin project, which this fork does not use.
const crowdinAPIURL = ""
...
// GetCrowdinTranslators returns an empty credits list. Upstream fetched Crowdin project
// members from tcno.co; this fork makes no such request.
func (*PlatformService) GetCrowdinTranslators() (CrowdinTranslatorsList, error) {
	if crowdinAPIURL == "" {
		return CrowdinTranslatorsList{ProofReaders: []CrowdinProofReader{}, Translators: []string{}}, nil
	}
