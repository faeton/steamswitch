...
import (
	...
	"steamswitch/internal/settingsfile"
)
...
type AppSettings struct {
	...
	// AppearanceMode is the System/Light/Dark axis of the built-in base theme:
	// "system" (default), "light" or "dark". Orthogonal to Theme, which selects a
	// classic theme pack and, when set, supplies its own fixed palette.
	...
	// UserDataPath is the absolute path to the SteamSwitch user data folder.
	UserDataPath string `json:"userDataPath,omitempty"`

	// ProtocolEnabled registers the steamswitch:// URL scheme on Windows when true.
...
// LoadAppSettings reads SteamSwitch.settings.json from the resolved location.
func LoadAppSettings(exeDir string) (AppSettings, error) {
	return loadSettings(exeDir)
}

// SaveAppSettings writes SteamSwitch.settings.json atomically.
