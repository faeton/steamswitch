<script lang="ts">
  ...
  // Mirrors steam.SteamAutoRefreshMinMinutes; the backend clamps regardless.
...
<SharedSettingCheckbox
  id="ps-remember-password"
  checked={steamSettings.Steam_RememberPassword}
  label={$t("Settings_SteamRememberPassword")}
  tooltip={$t("Settings_SteamRememberPassword_Tooltip")}
  on:change={() => {
    steamSettings.Steam_RememberPassword = !steamSettings.Steam_RememberPassword;
    dispatch("save");
  }}
/>
<SharedSettingCheckbox
...
<SharedSettingCheckbox
  id="ps-auto-refresh-launch"
  checked={steamSettings.Steam_AutoRefreshOnLaunch}
  label={$t("Settings_Steam_AutoRefreshOnLaunch")}
  on:change={() => {
    steamSettings.Steam_AutoRefreshOnLaunch = !steamSettings.Steam_AutoRefreshOnLaunch;
    dispatch("save");
  }}
/>
<div class="rowSetting">
  <label for="ps-auto-refresh-interval">{$t("Settings_Steam_AutoRefreshInterval")}</label>
  <input
    id="ps-auto-refresh-interval"
    type="number"
    min="0"
    step="5"
    bind:value={steamSettings.Steam_AutoRefreshIntervalMinutes}
    on:change={() => dispatch("save")}
  />
  <p class="subtext">
...
<div class="rowSetting">
  <label for="ps-image-expiry">{$t("Settings_Steam_ImageExpiry")}</label>
  <input
    id="ps-image-expiry"
    type="number"
    min="0"
