...
const ROUTES: Route[] = [
  ...
  { page: "vault" },
  { page: "about" },
  { page: "settings", category: "appearance" },
  { page: "settings", category: "vault" },
  { page: "tools" },
  { page: "preview-css" },
...
describe("validateRoute", () => {
  ...
  it("passes platform-independent routes through untouched", () => {
    const route: Route = { page: "settings", category: "appearance" };
    expect(validateRoute(route, startup({ allPlatformNames: [] }))).toBe(route);
  });

  // The vault keys its entries by SteamID64, so without Steam there is nothing for it to
  // describe — same treatment as the other Steam-dependent routes.
  it("drops the vault when Steam is unavailable", () => {
    expect(validateRoute({ page: "vault" }, startup({ disabledPlatformNames: ["Steam"] }))).toEqual({
      page: "home",
    });
  });
});
...
describe("settings categories", () => {
  it("defaults a bare #/settings to Appearance rather than failing", () => {
    expect(parseHash("#/settings")).toEqual({ page: "settings", category: "appearance" });
  });

  it("round-trips each category", () => {
    for (const category of SETTINGS_CATEGORIES) {
      const route: Route = { page: "settings", category };
      expect(parseHash(serializeRoute(route))).toEqual(route);
    }
  });

  // Rejected rather than silently coerced: a typo landing on Appearance would look like the
  // link worked, and the user would never learn the URL was wrong.
  it("rejects an unknown category", () => {
    expect(parseHash("#/settings/nope")).toBeNull();
  });

  it("lands the legacy platform-settings alias on a real category", () => {
    expect(parseHash("#/platform-settings/Steam")).toEqual({
      page: "settings",
      category: "appearance",
    });
  });
